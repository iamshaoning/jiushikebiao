    ...
    return (
      <div className="w-full max-w-md animate-slide-up-slow">
        <div className="text-center">
          ...
          <h2 className="font-display text-xl font-bold text-ink-700 mb-2">注册成功</h2>
          <p className="text-sm text-gray-500 leading-relaxed mb-6">
            您的账号已创建成功，现在可以登录开始排课了。
            <br />
            点击下方按钮前往登录。
          </p>
...
export default function AuthPage() {
  ...
  const getLayout = () => {
    ...
    if (isMobile) {
      ...
      // 形变后位移至顶栏位置：translate(12-32, 10-32) = (-20, -22)，并 scale(0.5) 缩到顶栏 logo 尺寸
      const logoAtBar = 'translate(-20px, -22px) scale(0.5)';
      const logoAtIdle = 'translate(0px, 0px) scale(1)';
      // 文字额外缩放：idle 文字 text-base(16px)，顶栏文字 text-sm(14px)
      // 容器整体 scale(0.5) 后文字视觉 8px，需额外 scale(1.75) 抵消到 14px，精确匹配顶栏文字尺寸
      // 配合 transformOrigin: left center，文字左边对齐顶栏文字左边（logo 右 gap-1.5 处）
    ...
    // 桌面端：左右分栏，形变目标为系统页侧边栏宽度占比（desktop:w-64 = 256px）
  ...
  // logo 旁的品牌文字始终跟随 logo 容器的 transform 一起位移缩放，到达目标位置，不单独淡出消失
    ...
    <div className="min-h-screen flex flex-col desktop:flex-row bg-cream">
      ...
      >
        ...
        <div className="relative flex flex-col justify-between h-full pointer-events-none">
          ...
          >
            <h1 className="font-display text-4xl desktop:text-5xl font-bold leading-tight text-white">排课 · 管理</h1>
          </div>
