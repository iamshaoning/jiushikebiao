...
export class StatisticsRenderService {
    ...
    orgTable(data, totalCourses, utils) {
        var tb = this.elements.organizationTable; if (!tb) return; tb.innerHTML = '';
        ...
        Object.entries(data).sort(function([,a],[,b]) { return b.courses - a.courses; }).forEach(function([org, stats]) {
            var pct = totalCourses > 0 ? (stats.courses / totalCourses * 100) : 0, color = utils.generateColor(org, 'organization'), row = document.createElement('tr');
    ...
    _renderTable(title, headers, rows) {
        var h = '<h4 class="text-md font-medium mb-3" style="color:var(--text-primary)">' + title + '</h4>';
        if (!rows.length) return '<div class="mb-6">' + h + '<div class="text-center py-8" style="color:var(--text-secondary)"><p>暂无数据</p></div></div>';
        var th = ''; headers.forEach(function(hdr) { th += '<th class="px-4 py-3 text-left text-base font-semibold" style="color:var(--text-secondary)">' + hdr + '</th>'; });
    ...
    _renderOneOnOneTable(stats, utils) {
        ...
        var orgGradeStats = {}; Object.entries(stats).forEach(function([grade, orgStats]) { Object.entries(orgStats).forEach(function([org, s]) { if (!orgGradeStats[org]) orgGradeStats[org] = {}; orgGradeStats[org][grade] = s; }); });
    ...
    _renderGroupTable(stats, utils) {
        ...
        var orgGradeCountStats = {}; Object.entries(stats).forEach(function([sc, gs]) { Object.entries(gs).forEach(function([grade, os]) { Object.entries(os).forEach(function([org, s]) { if (!orgGradeCountStats[org]) orgGradeCountStats[org] = {}; if (!orgGradeCountStats[org][grade]) orgGradeCountStats[org][grade] = {}; orgGradeCountStats[org][grade][sc] = s; }); }); });
    ...
    detailedTypeTable(detailedStats, utils) {
        var c = this.elements.detailedTypeTableContainer; if (!c) return;
