...
class ExportService {
    ...
    _buildExportRows(organizationStats, studentStats, detailedStats, state) {
        ...
        Object.entries(organizationStats).sort(([,a],[,b]) => b.courses - a.courses).forEach(([org, stats]) => rows.push({ 日期: org, 时间: stats.courses, 学生姓名: stats.fee.toFixed(2), 所属机构: stats.students.size, 年级: totalCourses > 0 ? (stats.courses / totalCourses * 100).toFixed(1) + '%' : '0%' }));
        ...
        Object.entries(orgGradeStats).sort(([a],[b]) => a.localeCompare(b)).forEach(([org, gradeStats]) => Object.entries(gradeStats).forEach(([grade, stats]) => rows.push({ 日期: org, 时间: grade, 学生姓名: stats.students.size, 所属机构: stats.courses, 年级: stats.fee.toFixed(2) })));
