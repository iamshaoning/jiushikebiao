    ...
    async show() {
        ...
        const generateTimelineHtml = () => {
            ...
            return timeline.map((action, index) => {
                ...
                return `<div class="timeline-item ${action.undone ? 'timeline-item-undone' : ''}" data-id="${action.id}" style="position: relative; padding-left: 32px; padding-bottom: 16px;">
                    ...
                    <div class="timeline-content" style="border: 1px solid var(--border-color); border-radius: 10px; padding: 10px 12px; ${action.undone ? 'opacity: 0.6;' : ''}">
                        <div class="timeline-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 6px;">
                            ...
                            <div class="timeline-time" style="font-size: 11px; color: var(--text-secondary); white-space: nowrap; background-color: var(--bg-tertiary); padding: 2px 8px; border-radius: 10px; flex-shrink: 0; margin-left: 8px;">${formatTimestamp(action.timestamp)}</div>
                        </div>
                        ...
                        <div class="timeline-actions" style="display: flex; gap: 6px; margin-top: 8px;">${action.undone ? `<button data-action="redo-timeline-action" data-id="${action.id}" class="timeline-action-btn" style="display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; font-size: 12px; font-weight: 500; border-radius: 6px; cursor: pointer; background-color: rgba(34,197,94,0.1); color: var(--color-success); border: none;"><i data-lucide="rotate-ccw" class="inline-block" style="width: 14px; height: 14px;"></i>重做</button>` : `<button data-action="undo-timeline-action" data-id="${action.id}" class="timeline-action-btn" style="display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; font-size: 12px; font-weight: 500; border-radius: 6px; cursor: pointer; background-color: rgba(59,130,246,0.1); color: var(--color-primary); border: none;"><i data-lucide="undo-2" class="inline-block" style="width: 14px; height: 14px;"></i>撤销</button>`}</div>
                    </div>
                </div>`;
            }).join('');
        };

        const content = `<div class="rounded-lg shadow-xl w-full max-w-lg mx-4" style="background-color: var(--bg-secondary);"><div class="p-4"><div class="flex justify-between items-center mb-4"><div><h3 class="text-base font-semibold" style="color: var(--text-primary); display: flex; align-items: center; gap: 6px;"><i data-lucide="history" class="inline-block" style="width: 18px; height: 18px;"></i>操作历史</h3><p class="text-xs" style="color: var(--text-secondary); margin-top: 2px;">最多保存 ${registry.get('timelineService').maxRecords}条记录</p></div><button id="clear-timeline-btn" style="display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; font-size: 12px; font-weight: 500; border-radius: 6px; cursor: pointer; background-color: rgba(239,68,68,0.1); color: var(--color-danger); border: none;"><i data-lucide="trash" class="inline-block" style="width: 14px; height: 14px;"></i>清空历史</button></div><div id="timeline-container" class="max-h-[50vh] overflow-y-auto" style="padding-right: 4px;">${generateTimelineHtml()}</div></div></div>`;
