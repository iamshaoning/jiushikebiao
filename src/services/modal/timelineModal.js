...
export class TimelineModal {
    ...
    async show() {
        ...
        const generateExpandedCourses = (action) => {
            ...
            if (courses.length <= 1) {
                return courses.map(c => registry.get('timelineService').generateCourseTag(c, [], true)).join('');
            }
            return `<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px;">${courses.map(c => registry.get('timelineService').generateCourseTag(c, [], true)).join('')}</div>`;
        };
        ...
        const generateTimelineHtml = () => {
            ...
            return timeline.map((action, index) => {
                ...
                const courseTag = (action.course || action.newCourse) ? registry.get('timelineService').generateCourseTag(action.course || action.newCourse, action.changes || [], true) : '';
                const oldCourseTag = action.oldCourse ? registry.get('timelineService').generateCourseTag(action.oldCourse, action.changes || [], true) : '';
                ...
                return `<div class="timeline-item ${action.undone ? 'timeline-item-undone' : ''}" data-id="${action.id}" style="position: relative; padding-left: 36px; padding-bottom: 20px;">
                    ...
                    <div class="timeline-content" style="border: 1px solid var(--border-color); border-radius: 10px; padding: 10px 12px; ${action.undone ? 'opacity: 0.6;' : ''}">
                        ...
                        ${action.type === 'update-course' && oldCourseTag && courseTag ? `<div class="timeline-course-compare" style="margin-top: 8px; display: flex; align-items: flex-start; gap: 6px;"><div style="flex: 1; min-width: 0;"><div style="font-size: 10px; color: var(--color-danger); margin-bottom: 2px;">修改前</div>${oldCourseTag}</div><div style="display: flex; align-items: center; padding-top: 22px; flex-shrink: 0;"><i data-lucide="arrow-right" class="inline-block" style="width: 14px; height: 14px; color: var(--text-secondary);"></i></div><div style="flex: 1; min-width: 0;"><div style="font-size: 10px; color: var(--color-success); margin-bottom: 2px;">修改后</div>${courseTag}</div></div>` : ''}
