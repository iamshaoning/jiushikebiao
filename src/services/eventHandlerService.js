...
class EventHandlerService {
    ...
    _setupUIHandlers() { return {
        ...
        'toggle-duration-dropdown': (p, e) => { if (!e || !e.target || e.target.closest('#duration-dropdown')) return; const u = registry.get('utils'); u.toggleDurationPicker('duration-dropdown'); },
        'select-duration': (p, e) => { if (!e || !e.target) return; const b = e.target.closest('[data-duration]'); if (!b) return; const d = parseInt(b.dataset.duration); if (isNaN(d)) return; const el = document.getElementById('course-duration'); if (el) el.value = d; const sti = document.getElementById('course-start-time'); const st = sti?.value || ''; const u = registry.get('utils'); if (st) u.calculateEndTime('course-start-time', 'course-end-time', d); u.calculateFee(); const dd = document.getElementById('duration-dropdown'); if (dd) { dd.classList.add('hidden'); const cs = registry.get('customSelectService'); if (cs?.closeListener) { document.removeEventListener('click', cs.closeListener); cs.closeListener = null; } } },
